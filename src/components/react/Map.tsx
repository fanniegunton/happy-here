...
interface MapProps {
  ...
  happyHourStatus: Record<string, boolean>;
  theme: Theme;
}

const Map = ({ establishments, happyHourStatus, theme }: MapProps) => {
  ...
  useEffect(() => {
    ...
    const loadMap = async () => {
      ...
      let center: [number, number] = [30.2672, -97.7431]; // Austin default
      if (validEstablishments.length > 0) {
        const avgLat =
          validEstablishments.reduce((sum, est) => sum + (est.location?.lat || 0), 0) /
          validEstablishments.length;
        const avgLng =
          validEstablishments.reduce((sum, est) => sum + (est.location?.lng || 0), 0) /
          validEstablishments.length;
  ...
  useEffect(() => {
    ...
    validEstablishments.forEach((establishment) => {
      if (!establishment.location) return;
      ...
      const marker = leaflet
        .circleMarker([establishment.location.lat, establishment.location.lng], {
          radius: 8,
